...
> Deviation log (decided 2026-08-14, founder-approved): §3.4 "suggested stack" is superseded by
> `RECIPE.md` Part A7 — pnpm + Turborepo monorepo, NestJS (Clean Architecture), Drizzle + Postgres,
> Better Auth, BullMQ + Redis. §3.4 self-labels as an assumption, so this is a sanctioned override.
> Model ids in §17 are stale; current ids are `claude-opus-5`, `claude-sonnet-5`,
> `claude-haiku-4-5-20251001`. Everything else in this file is the contract.
...
The system should automate the majority of coding and operational work: cron, webhooks, Kanban tks, and open-ended goal loops. After each session the container is destroyed. The next session reinitializes cleanly.
...
- Game-project specifics (`fight-for` is only an example of a mounted repo)
...
2. An **Agent** is a named role with a one-job contract, a scoped tool grant, and a runner preference.
3. A **Task** is a Kanban card: assignee agent, description, attachments, schedule, approval gate, chain position.
...
**SUPERSEDED — see deviation log at top of file. RECIPE.md A7 is the binding stack.**

| Concern | Assumption (original) | Binding (RECIPE A7) |
|---|---|---|
| Language | TypeScript (Node 20+) | TypeScript (Node 24) |
| API | Hono or Fastify on a single service | NestJS, Clean Architecture, `apps/api` |
| DB | Postgres + Prisma (or Drizzle) | Postgres + Drizzle, `packages/db` |
| Queue / cron | pg-boss or Inngest | BullMQ + Redis |
| Agent runtime (cloud) | Anthropic Claude Agent SDK / Claude managed agents | same |
| Agent runtime (local) | Worker VM running Claude Code / Grok yolo | same |
| Files | Cloudflare R2 + custom MCP enforcing ACLs | same (S3-compatible; MinIO for local dev) |
| Secrets | Google Secret Manager / Cloud KMS | provider interface; env-backed driver for dev |
| Auth | Single-operator, cookie or PAT | Better Auth, single operator |
| UI | React + Vite sidebar app, PWA | Vite + React + TanStack Router/Query + Tailwind + shadcn/ui |
| Webhooks | Public HTTPS receiver, per-trigger secrets | same |
| Push | Web Push (VAPID) | same |
...
- `title`
- `model` (e.g. a Claude model for planners; Grok for workers — routing is also a runner concern)
...
| Agent.runnerPreference = `local` (workers) | Grok / local Claude, fast |
...
Example shape (illustrative, reconstructed; model ids updated to current):
...
```ts
// types — implement as Drizzle models (RECIPE A7 supersedes Prisma)
